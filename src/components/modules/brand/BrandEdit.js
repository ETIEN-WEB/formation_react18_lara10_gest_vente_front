...
const BrandEdit = () => {
    ...
    const getBrand = () =>{
        axiosClient.get(`/brand/${params.id}`).then(res=>{
    ...
    const handleBrandUpdate = () => {
        setIsLoading(true)
        axiosClient.put(`/brand/${params.id}`, input).then(res=>{
            setIsLoading(false)
            Swal.fire({
                position: "top-end",
                icon: res.data.cls,
    ...
    return (
        <>
            <BreadCrumb title={'Modifier la marque'} />
            <div className="row ">
                                        ...
                                        <div className='row'>
                                            <div className="col-6">
                                                <div className="photo-preview mt-3">
                                                    <img src={input.logo == undefined ? input.logo_preview : input.logo } alt={'Logo preview'} className={'img-thumbnail aspect-one'} />
                                                </div>
                                            </div>
                                        </div> : null
                                    }
                                    ...
                                    <div className="row justify-content-center">
                                        <div className="col-md-4">
                                            <div className={'d-grid mt-4'}>
                                                <button className={'btn theme-button'} onClick={handleBrandUpdate} dangerouslySetInnerHTML={{__html: isLoading ? ' <span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span> Chargement... ' : ' Modifier une marque ' }}/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
