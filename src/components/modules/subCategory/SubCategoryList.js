...
const SubCategoryList = () => {
    ...
    const getCategories = (pageNumber=1) => {
        setIsLoading(true)
        axiosClient.get(`/sub-category?page=${pageNumber}&search=${input.search}&order_by=${input.order_by}&per_page=${input.per_page}&direction=${input.direction}`).then( res => {
    ...
    const handleCategoryDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axiosClient.delete(`/sub-category/${id}`).then(res=>{
                    getCategories()
                    Swal.fire({
                        position: "top-end",
                        icon: res.data.cls,
    ...
    return (
        <>
            <BreadCrumb title={'Liste des sous-catégories'} />
            <div className="row ">
                {/* <pre> {JSON.stringify(sous-categories, undefined, 2)} </pre> */}
                                        ...
                                        {Object.keys(categories).length > 0 ? categories?.map((category, index) => (
                                            <tr key={index} >
                                                ...
                                                <td>
                                                    ...
                                                    <p className={'text-theme'}> Catégorie: {category.category_name} </p>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    ...
                                                    <Link to={`/sub-category/edit/${category.id}`} > <button className={'btn btn-sm btn-warning my-1'}> <i className="fa-solid fa-edit"/> </button></Link>
