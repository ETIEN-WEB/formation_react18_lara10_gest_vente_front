...
const CategoryEdit = () => {
    ...
    const getCategory = () =>{
        axiosClient.get(`/category/${params.id}`).then(res=>{
    ...
    const handleCategoryUpdate = () => {
        setIsLoading(true)
        axiosClient.put(`/category/${params.id}`, input).then(res=>{
            setIsLoading(false)
            Swal.fire({
                position: "top-end",
                icon: res.data.cls,
    ...
    return (
        <>
            <BreadCrumb title={'Modifier une categorie'} />
            <div className="row ">
                                    ...
                                    <div className="row justify-content-center">
                                        <div className="col-md-4">
                                            <div className={'d-grid mt-4'}>
                                                <button className={'btn theme-button'} onClick={handleCategoryUpdate} dangerouslySetInnerHTML={{__html: isLoading ? ' <span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span> Chargement... ' : ' Modifier une catégorie ' }}/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
