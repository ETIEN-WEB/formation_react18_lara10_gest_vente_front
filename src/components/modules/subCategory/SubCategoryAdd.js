...
const SubCategoryAdd = () => {
    ...
    const handleCategoryCreate = () => {
        setIsLoading(true)
        axiosClient.post('/sub-category', input).then(res=>{
            setIsLoading(false)
            Swal.fire({
                position: "top-end",
                icon: res.data.cls,
    ...
    return (
        <>
            <BreadCrumb title={'Ajouter une sous categorie'} />
            <div className="row ">
                                    ...
                                    <div className="row justify-content-center">
                                        <div className="col-md-4">
                                            <div className={'d-grid mt-4'}>
                                                <button className={'btn theme-button'} onClick={handleCategoryCreate} dangerouslySetInnerHTML={{__html: isLoading ? ' <span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span> Chargement... ' : ' Ajouter une sous-catégorie ' }}/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
